PORT ?= 8000
CONTAINER := sentiment_classification
VERSION ?= `cat VERSION`
NUM_WORKERS ?= 1
DEMO_NAME ?= new-demo
LOG_LEVEL ?= DEBUG
...
.PHONY: test
test:
	pytest sentiment_classification
	flake8 sentiment_classification --exclude *_pb2.py


# optional; if protobuf schema is used
.PHONY: proto
proto:
	protoc --python_out=sentiment_classification schema.proto


# run service locally
.PHONY: run
run:
	gunicorn -b:$(PORT) 'sentiment_classification.serving:run_app()' --log-level=$(LOG_LEVEL)
